-- ============================================================================
-- CoinTracer — Production-Safe Schema Migration
-- ============================================================================
-- This migration is safe to run against a fresh OR existing database.
-- - Uses CREATE TABLE IF NOT EXISTS (never drops tables)
-- - Uses CREATE INDEX IF NOT EXISTS
-- - Uses CREATE OR REPLACE for views/functions
-- - Duplicate alerts table definition removed
-- ============================================================================
...
-- Conditional unique index (can't use IF NOT EXISTS on unique indexes in all PG versions)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'uniq_exchange_api_key_hash') THEN
        CREATE UNIQUE INDEX uniq_exchange_api_key_hash
...
-- Add unique constraint on (portfolio_id, trade_id) if not already present
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'unique_tx'
    ) THEN
...
-- Single alerts table definition (removed duplicate from original schema)
